...
#nicep arr
#puts
(0..n-1).each do |currcol|
  newarr = Marshal.load(Marshal.dump(arr))
  newarr = []
  (0..n-1).each do |i|
    minarr = [0.0]*n
    newarr.push minarr.dup
  end

  (0..n-1).each do |currrow|

    currprob = arr[currrow][currcol]
#    newarr[currrow][currcol] = 0

#    (0..n-1).each do |fliprow|
#      (0..n-1).each do |flipcol|
#        if (fliprow!=currrow and flipcol!=currcol)
#          newarr[fliprow][flipcol]=0
#        end
#      end
#    end
	...
	if fliprow == currrow
          if flipcol == currcol
#            newarr[fliprow][flipcol]-=currprob
            newarr[fliprow][flipcol]+=currprob/nf
          else
       	    newarr[fliprow][flipcol]+= currprob/nf
          end
        else
	  if flipcol==currcol
            newarr[fliprow][flipcol]+= currprob/nf #some conditional probability here
          else
            asd = (arr[fliprow][flipcol]/(1-arr[fliprow][currcol]))
            if asd.nan?
              asd = 0
            end
	    newarr[fliprow][flipcol]+= asd*currprob*((nf-1)/nf)
	    #newarr[fliprow][flipcol]-= *((nf-1)/nf)
          end
        end
      end
    end
...
#nicep arr.map{|x| x.map{|z|(z*n**n-n**(n-1)).round}}
#arr = arr.map{|a| a.map{|x| Math.exp(x)}}

barr = Marshal.load(Marshal.dump(arr))
...
#p barr
n = Munkres.new(barr,1)

asd = Array.new

n.find_pairings.each do |pair|
  asd[pair[1]] = pair[0]+1
end
